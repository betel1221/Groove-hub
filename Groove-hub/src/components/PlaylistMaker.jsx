// src/components/PlaylistMaker.jsx
...
function PlaylistMaker() {
  ...
  // Save playlists to Local Storage whenever userPlaylists state changes
  useEffect(() => {
  ...
  const handleCloseModal = () => {
    setShowModal(false);
    setNewPlaylistName(''); // Clear input on close
  };

  const handleCreatePlaylist = () => {
    if (newPlaylistName.trim()) {
      const newPlaylist = {
        id: `playlist-${Date.now()}`, // Unique ID for the playlist
        name: newPlaylistName.trim(),
        songs: [], // Playlists start empty
        // You could add a default image here if you like
        // imageUrl: '/default-playlist-cover.jpg',
      };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 p-8 rounded-lg shadow-xl w-full max-w-md border border-gray-700">
            ...
            <input
              ...
              onKeyDown={(e) => { // Allow creating playlist on Enter key press
                if (e.key === 'Enter') {
